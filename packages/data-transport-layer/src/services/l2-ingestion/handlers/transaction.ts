...
export const handleSequencerBlock = {
  parseBlock: async (
    block: { transactions: Array<any>; stateRoot: string },
    chainId: number
  ): Promise<
    Array<{
  ...
  > => {
    const result = block.transactions.map((transaction) => {
  ...
  storeBlock: async (
    ...
    await db.putUnconfirmedTransactionEntries(
      entry.map((v) => v.transactionEntry)
    )
    await db.putUnconfirmedStateRootEntries(entry.map((v) => v.stateRootEntry))
  },
}
